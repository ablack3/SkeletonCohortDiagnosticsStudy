...
executeOnMultipleDataSources <- function(x) {
  ...
  if (x$generateCohortTableName) {
    cohortTableName <- paste0(stringr::str_squish(x$databaseId),
                              stringr::str_squish("SkeletonCohortDiagnosticsStudy"))
  }

  # this function gets details of the data source from cdm source table in omop, if populated.  The
  # assumption is the cdm_source.sourceDescription has text description of data source.

  getDataSourceInformation <- function(connectionDetails,
                                       cdmDatabaseSchema,
                                       vocabDatabaseSchema,
                                       databaseId) {
    sqlCdmDataSource <- "select * from @cdmDatabaseSchema.cdm_source;"
    sqlVocabularyVersion <- "select * from @vocabDatabaseSchema.vocabulary where vocabulary_id = 'None';"
    etlVersionNumber <- "select * from @cdmDatabaseSchema._version;"
    sourceInfo <- list(cdmSourceName = databaseId, sourceDescription = databaseId)
    ...
    vocabulary <- DatabaseConnector::renderTranslateQuerySql(connection = connection,
      ...
      vocabDatabaseSchema = vocabDatabaseSchema, snakeCaseToCamelCase = TRUE) %>%
      dplyr::tibble() %>%
      dplyr::rename(vocabularyVersion = .data$vocabularyVersion)

    tryCatch(expr = {
      cdmDataSource <- DatabaseConnector::renderTranslateQuerySql(connection = connection,
                                                                  sql = sqlCdmDataSource,

        cdmDatabaseSchema = cdmDatabaseSchema, snakeCaseToCamelCase = TRUE)
      ...
      if ("cdmEtlReference" %in% colnames(cdmDataSource)) {
        sourceInfo$sourceDescription <- paste0(sourceInfo$sourceDescription, " ETL Reference: ",
      ...
      if ("cdmReleaseDate" %in% colnames(cdmDataSource)) {
        sourceInfo$sourceDescription <- paste0(sourceInfo$sourceDescription, " CDM release date: ",
    ...
    tryCatch(expr = {
      version <- DatabaseConnector::renderTranslateQuerySql(connection = connection,
    ...
    if (nrow(version) == 1) {
      if (all("versionId" %in% colnames(version), "versionDate" %in% colnames(version))) {
        cdmDataSource <- cdmDataSource %>%
          dplyr::mutate(cdmSourceAbbreviation = paste0(.data$cdmSourceAbbreviation,
                                                       " (v",
                                                       version$versionId,

          " ", as.character(version$versionDate), ")")) %>%
          dplyr::mutate(cdmSourceName = paste0(.data$cdmSourceName, " (v", version$versionId,
          " ", as.character(version$versionDate), ")")) %>%
          dplyr::mutate(sourceDescription = paste0(.data$sourceDescription,
                                                   " (v",
                                                   version$versionId,
    ...
    return(if (nrow(cdmDataSource) > 0) {
      tidyr::crossing(cdmDataSource, vocabulary, version) %>%
        dplyr::mutate(databaseDescription = .data$sourceDescription)
    } else {
      warning("cdmDataSource table has no data")
      vocabulary
    })
  ...
  connectionDetails <- DatabaseConnector::createConnectionDetails(dbms = x$cdmSource$dbms,
                                                                  server = x$cdmSource$server,

    user = keyring::key_get(service = x$userService), password = keyring::key_get(service = x$passwordService),
  ...
  dataSourceDetails <- getDataSourceInformation(connectionDetails = connectionDetails,
                                                cdmDatabaseSchema = cdmDatabaseSchema,

    vocabDatabaseSchema = vocabDatabaseSchema, databaseId = x$databaseId)

  if (all(!is.null(dataSourceDetails),
          nrow(dataSourceDetails) == 1,
          nchar(dataSourceDetails$versionId) >
    0)) {
    databaseId <- paste0(x$databaseId,
                         "_v",
                         dataSourceDetails$versionId,
                         "_",
                         stringr::str_replace_all(string = as.character(dataSourceDetails$versionDate),
  ...
  SkeletonCohortDiagnosticsStudy::execute(connectionDetails = connectionDetails,
    ...
    cohortDatabaseSchema = cohortDatabaseSchema, cohortTable = cohortTableName, verifyDependencies = x$verifyDependencies,
    outputFolder = x$outputFolder, databaseId = databaseId, databaseName = dataSourceDetails$cdmSourceName,
