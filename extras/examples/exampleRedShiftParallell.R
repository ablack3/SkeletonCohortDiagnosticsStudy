...
# cdmSources <- cdmSources2 rm('cdmSources2') databaseIds to run cohort diagnostics on that source
# #################
databaseIds <- c("truven_ccae",
                 "truven_mdcd",
                 "cprd",
                 "jmdc",
                 "optum_extended_dod",
  ...
  "ims_australia_lpd", "ims_germany", "ims_france")
...
for (i in (1:length(databaseIds))) {
  ...
  x[[i]] <- list(cdmSource = cdmSource, generateCohortTableName = TRUE, verifyDependencies = FALSE,
    databaseId = databaseIds[[i]], outputFolder = file.path(outputFolder,
                                                            databaseIds[[i]]), userService = keyringUserService,
    passwordService = keyringPasswordService, preMergeDiagnosticsFiles = TRUE)
}
...
## file logger
loggerName <- paste0("CDF_",
                     stringr::str_replace_all(string = Sys.time(),
                                              pattern = ":|-|EDT| ",
                                              replacement = ""))
loggerTrace <- ParallelLogger::addDefaultFileLogger(fileName = file.path(outputFolder,
                                                                         paste0(loggerName,
  ".txt")))


ParallelLogger::clusterApply(cluster = cluster, x = x, fun = executeOnMultipleDataSources)
